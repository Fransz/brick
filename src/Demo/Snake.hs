...
isCrash p s
  | p `elem` snake s = s {gameover = True}
  | otherwise = s
...
addMove s m
  | m `elem` [SnakeUp, SnakeDown] && cur `elem` [SnakeUp, SnakeDown] = s
